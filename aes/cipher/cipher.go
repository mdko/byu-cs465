...
func Cipher(inState[4][constants.Nb]byte, w[constants.Nb * (constants.Nr + 1)][4]byte) (outState[4][constants.Nb]byte) {
		debug.DebugPrint("Encryption(Cipher):")
...
func InvCipher(inState[4][constants.Nb]byte, w[constants.Nb * (constants.Nr + 1)][4]byte) (outState[4][constants.Nb]byte) {
		debug.DebugPrint("Decryption(InvCipher):")
		debug.DebugPrint("Round Number: input")
		...
		state = AddRoundKey(state, roundKeyValue)
		for round := constants.Nr - 1; round >= 1; round-- {
				debug.DebugPrint(fmt.Sprintf("Round Number: %v", round))
				debug.DebugPrint(fmt.Sprintf("Start of Round: %x", state))

				state = InvShiftRows(state)
				debug.DebugPrint(fmt.Sprintf("After InvShiftRows: %x", state))

				state = InvSubBytes(state)
				debug.DebugPrint(fmt.Sprintf("After InvSubBytes: %x", state))

				roundKeyValue = GetCurrentKeyScheduleSubMatrix(w, round*constants.Nb, (round+1)*constants.Nb - 1)
				debug.DebugPrint(fmt.Sprintf("Round Key Value: %x", roundKeyValue))
				
				state = AddRoundKey(state, roundKeyValue)
				debug.DebugPrint(fmt.Sprintf("After AddingRoundKey: %x", state))
				
				state = ffmath.InvMixColumns(state)
		}
		debug.DebugPrint("Round Number: 0")
		debug.DebugPrint(fmt.Sprintf("Start of Round: %x", state))
...
func InvSubBytes(inState[4][constants.Nb]byte) (outState[4][constants.Nb]byte) {
		for x, row := range(inState) {
			for y, byt := range(row) {
				...
				msB := (byt & 0xF0) >> 4
				lsB := (byt & 0x0F)
				outState[x][y] = constants.InvSbox[msB][lsB]
			}
		}
		return
}
...
func InvShiftRows(inState[4][constants.Nb]byte) (outState[4][constants.Nb]byte) {
		for r := 0; r < 4; r++ {
			for c := 0; c < constants.Nb; c++ {
				 outState[(c + r) % constants.Nb][r] = inState[c][r]// I needed to switch the [] order due to how I have set up my matrix (the columns are each row in my matrix)
			}
		}
		return
}