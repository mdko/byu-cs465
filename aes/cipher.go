...
const Nb = 4	// Number of columns (32 bit words) comprising the State; here, always 4
const Nk = 4	// Number of 32-bit words comprising the Cipher Key
const Nr = 10	// Number of rounds, which is 10
var Sbox = [][]byte {
...
byte[4, Nb] MixColumns(byte in_state[4, Nb]) {
	byte out_state[4, Nb]
	for c = 0 step 1 to Nb-1 { // for each column (0 to 3)
	out_state[0, c] = ffAdd(
						ffAdd(
							ffAdd(	ffMultiply({02}, in_state[0, c]),
									ffMultiply({03}, in_state[1, c])),
							in_state[2, c]),
						in_state[3, c])
	out_state[1, c] = ffAdd(
						ffAdd(
							ffAdd(	in_state[0, c],
									ffMultiply({02}, in_state[1, c])),
							ffMultiply({03}, in_state[2, c])),
						in_state[3, c])
	out_state[2, c] = ffAdd(
						ffAdd(
							ffAdd(	in_state[0, c],
									in_state[1, c]),
							ffMultiply({02}, in_state[2, c])),
						ffMultiply({03{, in_state[3, c]))
	out_state[3, c] = ffAdd(
						ffAdd(
							ffAdd(	ffMultiply({03}, in_state[0, c]),
									in_state[1, c]),
							in_state[2, c]),
						ffMultiply({02}, in_state[3, c]))
	}
	return out_state
}


byte ffAdd(byte l_byte, byte r_byte) {
	return XOR(l_byte, r_byte)
}

byte ffMultiply(byte l_byte, byte r_byte) {
   ...
   // described in the slides:
	byte result = 0
	for each 1 bit current bit in binary representation(r_byte) {
		byte ceiling = value of current bit
		byte current_result = l_byte
		byte i = {02}
		for i < l_byte {
			current_result = xtime(result)
			i = i * {02}
		}
...
byte xtime(byte in) {
	overflow := in & 0x80	// The msb is set, so multiplying by two will overflow into a 2nd byte
	result := in << 1
	if overflow
		result = current_byte ^ 0x011b
	return result
}
